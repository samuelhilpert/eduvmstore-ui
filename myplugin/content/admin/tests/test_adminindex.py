...
@mock.patch("myplugin.content.admin.view.index.get_user_details",
            return_value={"role": {"access_level": 999}})
@mock.patch("myplugin.content.admin.view.index.get_users", return_value=[{"id": "user-123"}])
@mock.patch("myplugin.content.admin.view.index.get_roles",
            return_value=[{"name": "EduVMStoreAdmin", "access_level": 999}])
@mock.patch("myplugin.content.admin.view.index.get_app_templates_to_approve",
            return_value=[{"creator_id": "user-123"}])
...
@mock.patch("myplugin.content.admin.view.index.get_users", return_value=[{"id": "user-123"}])
@mock.patch("myplugin.content.admin.view.index.get_roles",
            return_value=[{"name": "SomeOtherRole", "access_level": 5}])
...
def test_context_data_user_details_exception(
        mock_templates, mock_username, mock_approve, mock_roles, mock_users, mock_user_details,
        view_instance):
    context = view_instance.get_context_data()
    assert context["show_content"] is False
...
@mock.patch("myplugin.content.admin.view.index.get_roles",
            return_value=[{"name": "EduVMStoreAdmin", "access_level": 999}])
@mock.patch("myplugin.content.admin.view.index.get_app_templates_to_approve",
            side_effect=Exception("approval API down"))
@mock.patch("myplugin.content.admin.view.index.get_username_from_id", return_value="admin_user")
@mock.patch("myplugin.content.admin.view.index.get_app_templates", return_value=[])
